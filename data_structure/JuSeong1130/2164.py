...
f(1,k) = 1이란게 명확함
점화식은 다음과 같이 구할 수 있다. f(n, k) = (f(n-1,k) + k - 1) % n + 1  (k 에 -1을 한것은 인덱스 때문) 맨마지막 + 1을 해준 이유는 f(n-1,k) + k - 1) % n을하면  
...
2^n + k 인경우다 예를들어 6이면 2^2승에다가 + 2를 하는 경우인데 k를 없앨라면 k번 동작을해야한다. 그러면 남은 숫자는 2^n이다.
즉 K개 동작을 한다는것은 2*k만큼 이동을 한다는 것인데 이동한 값 전이 닶이다.
2^N개 의 숫자가 남게 된다. k개 만큼의 수를 제거하면 = 한번의 이동이 버리고 뒤로 빼고  = 전체이동횟수는 2k
k개를 버리면 2^n개를 맞춘다.  1 2 3 4 5 6 2k만큼 가면 시작위치는 5  이동작업을 해줘라 2 4 5 6이 되고 현재위치는 5니까 4가 되는것
즉 이동횟수 2k -1이 정답이다.

동작이란게 1개 버리고 1개 맨뒤로 보내라 
...
이때, K번째 사람을 제거했기 때문에, 시작숫자는 1부터가 아니라 K+1부터가 될 것이다.
