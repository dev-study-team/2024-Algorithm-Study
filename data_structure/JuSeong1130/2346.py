...
양수면 오른쪽으로 이동 > 맨처음큐를 맨뒤로보낸다.
음수면 왼쪽으로 이동 > 맨뒤에큐를 맨처음으로 보낸다.

또다른 방식
https://velog.io/@highcho/Algorithm-baekjoon-2346
rotate로 양수면 왼쪽으로 당겨주고
음수면 오른쪽으로 밀어준
    

여기서 반례 찾는법 배열에 관해
이동하는 배열이면 오른쪽으로 계속이동시켜보고 왼쪽으로 계속이동시켜보고 오른쪽 왼쪽 계속 나눠서 이동시켜보면된다.
4
-4 -4 -4 -4 이걸로 인덱스 아웃문제를 찾았고 해결하게 되었다.
...
그리고 또 잘못생각한게 있는데 현재 길이 - (현재위치 + 풍선에 있는 값) % 현재 길이 이렇게 구하면 된다 생각 하였는데 현재 길이를 미리 구해 뒤에서 부터 몇번 이동했는지 찾고 %를 이용해 값을 구해야 하는 문제였다
                            공식이 맞았다고 생각했었는데 아에 잘못된 공식이었던 것이다.
